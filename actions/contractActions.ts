...
export const verifyContract = async (contractId: string, token: string): Promise<boolean> => {
  try {
    ...
    // Cache invalidieren - neuer Contract verfügbar
...
// 🔹 CONTRACT NACH ID FINDEN (Alias für findContractById)
export const findContractById = async (contractId: string): Promise<Contract | null> => {
  const contractDoc = await getContractById(contractId);
  if (!contractDoc) {
    return null;
  }

  // Konvertiere ContractDocument zu Contract
  const { verified, ...contractData } = contractDoc;
...
export const getContractPreviewsInRadius = async (
  ...
  try {
    ...
    const cacheKey = CACHE_KEYS.CONTRACT_PREVIEWS(
      companyLatitude,
      companyLongitude,
      radiusKm,
      companyServices
    ...
    if (useCache && !lastDocId) {
      ...
      if (cached) {
        console.log(`Contract-Previews aus Cache geladen: ${cached.contracts.length} Contracts`);
        return {
          ...cached,
          contracts: cached.contracts.slice(0, limit) // Respektiere das Limit
        };
      }
    }

    console.log('Lade Contract-Previews frisch aus Datenbank...');
    ...
    q = query(q, firestoreLimit(limit * 5));
    ...
    // Speichere in Cache (nur wenn keine Pagination und genug Daten)
    if (useCache && !lastDocId && validContracts.length > 0) {
      // Speichere alle gefundenen Contracts, nicht nur das Limit
      const fullResult = {
        contracts: validContracts.sort((a, b) => (a.distance || 0) - (b.distance || 0)),
        hasMore: false,
        lastDocId: undefined
      };
      cacheManager.set(cacheKey, fullResult, CACHE_OPTIONS.CONTRACTS);
      console.log(`${validContracts.length} Contract-Previews im Cache gespeichert`);
    }
...
export const getContractsInRadius = async (
  ...
  try {
    ...
    // Cache-Key generieren
    const cacheKey = CACHE_KEYS.CONTRACTS_IN_RADIUS(
      companyLatitude,
      companyLongitude,
      radiusKm,
      companyServices
    ...
    if (useCache && !lastDocId) {
      ...
      if (cached) {
        console.log(`Contracts aus Cache geladen: ${cached.contracts.length} Contracts`);
        return {
          ...cached,
          contracts: cached.contracts.slice(0, limit) // Respektiere das Limit
        };
    ...
    console.log('Lade Contracts frisch aus Datenbank...');
    ...
    if (useCache && !lastDocId && validContracts.length > 0) {
      ...
      cacheManager.set(cacheKey, fullResult, CACHE_OPTIONS.CONTRACTS);
      console.log(`${validContracts.length} Contracts im Cache gespeichert`);
    }
...
export const purchaseContract = async (
  ...
  try {
    ...
    await addDoc(collection(database, "purchases"), {
      ...
      amount: await calculateContractPrice(contractData)
    });

    // Cache invalidieren - Contract ist nicht mehr verfügbar
    invalidateContractCaches();
    
    // Gekaufte Contracts Cache für dieses Unternehmen invalidieren
    const purchasedCacheKey = CACHE_KEYS.PURCHASED_CONTRACTS(companyId);
    cacheManager.delete(purchasedCacheKey);
...
export const getPurchasedContracts = async (
  companyId: string,
  useCache: boolean = true
): Promise<ContractDocumentClient[]> => {
  try {
    // Cache-Key generieren
    const cacheKey = CACHE_KEYS.PURCHASED_CONTRACTS(companyId);

    // Versuche aus Cache zu laden
    if (useCache) {
      const cached = cacheManager.get<ContractDocumentClient[]>(cacheKey);
      if (cached) {
        console.log(`Gekaufte Contracts aus Cache geladen: ${cached.length} Contracts`);
        return cached;
      }
    }

    console.log('Lade gekaufte Contracts frisch aus Datenbank...');
    ...
    // Speichere in Cache
    if (useCache) {
      cacheManager.set(cacheKey, contracts, CACHE_OPTIONS.PURCHASED_CONTRACTS);
      console.log(`${contracts.length} gekaufte Contracts im Cache gespeichert`);
    }
