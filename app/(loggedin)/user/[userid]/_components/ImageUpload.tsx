...
const ImageUploadComponent: React.FC = () => {
    ...
    useEffect(() => {
        if(companyData) {
            ...
            setImages((prev) =>
              [...fetchedImages, ...Array(3 - fetchedImages.length).fill(null)].slice(0, 3)
            );
        }
      }, [companyData]);
      ...
      const handleFileChange = async (index: number, file: File | null) => {
        ...
        const storageRef = ref(storage, `images/${companyData!.id!}/${file.name}-${Date.now()}`);
        ...
        uploadTask.on(
          ...
          async () => {
            ...
            // Update Firestore
            const companyDocRef: CompanyType = {...companyData!, images: [...companyData!.images!, downloadURL]}
      ...
      const handleDeleteImage = async (index: number) => {
        ...
        companyDocRef.images = companyData!.images!.filter((img) => img !== imageUrl);
