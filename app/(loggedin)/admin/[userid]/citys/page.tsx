    ...
    <div className='mx-auto max-w-5xl py-2 md:px-6 md:py-8'>
      ...
      <div className='mb-5 rounded-md border border-slate-200 bg-white p-4 shadow-sm'>
        ...
        <input
          ...
          className='h-11 w-full rounded-md border border-slate-300 px-3 text-base outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-100'
        />
      ...
      <div className='mb-6 grid gap-3 sm:grid-cols-2 md:grid-cols-3 md:gap-4 lg:grid-cols-4'>
        {visible.map((c, i) => {
          const slug = slugify(c);
          return (
            <div
              key={slug + "-" + (start + i)}
              className='flex min-h-16 items-center justify-between gap-3 rounded-md border border-slate-200 bg-white p-3 text-sm shadow-sm'>
              <span className='truncate' title={c}>
                {c}
              </span>
              <button
                className='shrink-0 rounded-md bg-slate-950 px-3 py-2 text-xs font-medium text-white hover:bg-blue-700'
                onClick={() => {
