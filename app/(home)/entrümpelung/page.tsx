...
export async function generateMetadata() {
  return {
    title:
      "Entrümpelung Olpe & 30 km Umkreis — Schnell & günstig | Umzugshelden",
    description:
...
const pricingFactors = [
  {
    title: "Volumen & Anzahl der Gegenstände",
    text: "Je mehr Möbel, Kartons und Hausrat vorhanden sind, desto mehr Zeit und Fahrzeugkapazität wird benötigt. Wir schätzen das bei der kostenlosen Besichtigung realistisch ein.",
  },
  {
    title: "Stockwerk & Zugangssituation",
    text: "Kein Aufzug, enge Treppenwege oder schwieriger Zugang zum Gelände können den Aufwand erhöhen. Das fließt transparent in das Angebot ein – ohne Überraschungen.",
  },
  {
    title: "Art der Gegenstände",
    text: "Herkömmlicher Hausrat, Sondermüll, Elektrogeräte oder Gefahrstoffe werden unterschiedlich entsorgt. Wir erfassen alles bei der Besichtigung und kalkulieren entsprechend.",
  },
  {
    title: "Wertanrechnung möglich",
    text: "Gut erhaltene Möbel, Metalle, Antiquitäten oder Sammlungen können den Preis erheblich reduzieren. In manchen Fällen ist eine Entrümpelung sogar kostenlos möglich.",
  },
];
      ...
      <div className='grid grid-cols-1 lg:grid-cols-2 gap-16 items-start'>
        <div className='flex flex-col gap-6'>
          ...
          <p className='font-body text-gray-600 leading-relaxed'>
            Eine Pauschalaussage zum Preis ist bei Entrümpelungen schwierig, da
            der Aufwand stark variiert. Deshalb bieten wir{" "}
            <strong className='text-navy'>immer eine kostenlose Besichtigung</strong>{" "}
            an, bevor wir ein verbindliches Festpreisangebot erstellen. Keine
            Schätzungen, keine Stundensätze.
          </p>
          <p className='font-body text-gray-600 leading-relaxed'>
            Diese Faktoren bestimmen den Preis einer Entrümpelung:
          </p>
          <div className='flex flex-col gap-4'>
            {pricingFactors.map((f) => (
              <div key={f.title} className='flex gap-3 items-start'>
                <CheckIcon className='text-primary flex-shrink-0 mt-1' size={20} />
                <div>
                  <p className='font-sans font-semibold text-navy'>{f.title}</p>
                  <p className='font-body text-gray-600 text-sm'>{f.text}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className='flex flex-col gap-5'>
          <div className='bg-navy rounded-xl p-6'>
            <p className='font-sans font-semibold text-white mb-3'>
              Wertanrechnung: So funktioniert das
            </p>
            <p className='font-body text-gray-300 text-sm leading-relaxed mb-4'>
              Wenn verwertbare Gegenstände vorhanden sind – Metalle, gut erhaltene
              Möbel, Antiquitäten oder Elektrogeräte – können diese gegen den
              Entrümpelungspreis verrechnet werden. In manchen Fällen übersteigt der
              Wert sogar die Entsorgungskosten. Wir zeigen Ihnen transparent, was
              womit verrechnet wird.
            </p>
            <div className='flex flex-col gap-2'>
              {[
                "Metalle & Schrott → Schrotthandel",
                "Gut erhaltene Möbel → Secondhand-Ankauf",
                "Antiquitäten → Bewertung & Ankauf",
                "Funktionierende Elektronik → Weiterverwertung",
              ].map((item) => (
                <div key={item} className='flex gap-2 items-center'>
                  <div className='w-1.5 h-1.5 rounded-full bg-primary flex-shrink-0' />
                  <p className='font-body text-gray-300 text-sm'>{item}</p>
                </div>
              ))}
            </div>
          </div>
          <div className='bg-gray-50 rounded-xl p-6 border border-gray-100'>
            ...
            <p className='font-body text-gray-600 text-sm leading-relaxed'>
              Nach der Besichtigung erhalten Sie am selben oder nächsten Tag ein
              verbindliches Festpreisangebot – inklusive aller Entsorgungskosten.
            </p>
...
const disposalCategories = [
  {
    emoji: "\u267b\ufe0f",
    title: "Recyclingfähige Materialien",
    text: "Metalle, Papier, Glas und Kunststoffe werden sortenrein getrennt und den entsprechenden Verwertungsanlagen zugeführt.",
  },
  {
    emoji: "\ud83d\udc9a",
    title: "Noch verwertbare Gegenstände",
    text: "Gut erhaltene Möbel, Kleidung und Haushaltswaren gehen an Sozialkaufhäuser oder karitative Einrichtungen – so landet Brauchbares nicht im Müll.",
  },
  {
    emoji: "\u26a1",
    title: "Elektroschrott",
    text: "Alte Geräte (Fernseher, Kühlschränke, Computer) werden fachgerecht gemäß ElektroG entsorgt – sicher, legal und umweltgerecht.",
  },
  {
    emoji: "\u26a0\ufe0f",
    title: "Sondermüll & Gefahrstoffe",
    text: "Farben, Lacke, Batterien und andere Sonderabfälle werden nicht im Hausmmüll entsorgt, sondern an zertifizierte Sondermüllstellen übergeben.",
  },
];
    ...
    <div className='container mx-auto px-4'>
      <div className='mb-10 text-center'>
        <h2 className='font-sans font-bold text-3xl md:text-4xl text-navy'>
          Was passiert mit Ihren Sachen?
        </h2>
        <p className='font-body text-gray-600 mt-3 max-w-2xl mx-auto'>
          Wir entsorgen nicht einfach alles. Jeder Gegenstand wird so behandelt,
          wie es sinnvoll und gesetzlich korrekt ist.
        </p>
      </div>
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6'>
        {disposalCategories.map((cat) => (
          <div
            key={cat.title}
            className='bg-white rounded-xl p-6 border border-gray-100 shadow-sm flex flex-col gap-3 text-center'>
            <span className='text-4xl'>{cat.emoji}</span>
            <h3 className='font-sans font-semibold text-navy'>{cat.title}</h3>
            <p className='font-body text-gray-600 text-sm leading-relaxed'>
              {cat.text}
            </p>
          </div>
      ...
      <p className='font-body text-gray-500 text-sm text-center mt-8 max-w-xl mx-auto'>
        Unser Ziel: möglichst wenig landet auf der Deponie. Nachhaltigkeit ist
        uns nicht nur ein Schlagwort – wir dokumentieren die Entsorgungswege
        auf Wunsch.
      </p>
    </div>
  </section>
