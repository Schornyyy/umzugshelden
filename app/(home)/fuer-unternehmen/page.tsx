...
export async function generateMetadata() {
  return {
    ...
    description: 'Du bist im Garten- und Landschaftsbau tätig und suchst nach Aufträgen? Finde jetzt passende Galabau-Aufträge direkt in deiner Nähe mit JobSmith.',
    openGraph: {
      title: 'Finde Garten- & Landschaftsbau Aufträge in deiner Nähe',
      description: 'Du bist im Garten- und Landschaftsbau tätig und suchst nach Aufträgen? Finde jetzt passende Galabau-Aufträge direkt in deiner Nähe mit JobSmith.',
    ...
    <div className='container mx-auto my-12 px-4'>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-2 gap-12 items-center'>
        ...
        <div className='flex flex-col gap-6'>
          ...
          <p className='text-base sm:text-lg text-center lg:text-left'>
            Du bist im Garten- und Landschaftsbau tätig und suchst nach neuen Aufträgen in deiner Region? Mit unserer
            Plattform findest du gezielt Galabau-Aufträge in deiner Nähe – schnell, unkompliziert und genau auf deine
      ...
      <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 my-12 justify-items-center mt-24'>
        {bulletPointsCompanyyCard.map((item, index) => (
          ...
          >
            <h3 className='text-lg sm:text-xl font-bold text-center'>{item.title}</h3>
            <p className='text-sm sm:text-base text-center'>{item.description}</p>
            <Image alt={item.title} src={item.iconPath} width={32} height={32} />
          </div>
        ))}
      </div>
    </div>
